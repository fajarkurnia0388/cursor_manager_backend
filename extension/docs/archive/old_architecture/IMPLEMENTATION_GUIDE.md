...
class SQLiteService {
  ...
  async createTables() {
    ...
    this.db.run(
      "CREATE INDEX IF NOT EXISTS idx_cookie_account ON cookies(account_id)"
    );
    this.db.run(
      "CREATE INDEX IF NOT EXISTS idx_card_active ON payment_cards(is_active)"
    );
  ...
  getStats() {
    ...
    const accountCount = this.exec("SELECT COUNT(*) as count FROM accounts")[0]
      .count;
    const cookieCount = this.exec("SELECT COUNT(*) as count FROM cookies")[0]
      .count;
    const cardCount = this.exec(
...
```json
{
  ...
  "web_accessible_resources": [
    {
      "resources": ["libs/sql-wasm.wasm", "libs/sql-wasm.js"],
      "matches": ["<all_urls>"]
    }
  ],
...
class AccountServiceV2 {
   ...
   */
  async getById(id) {
    const accounts = this.db.exec("SELECT * FROM accounts WHERE id = ?", [id]);
    ...
    const cookies = this.db.exec("SELECT * FROM cookies WHERE account_id = ?", [
      id,
    ]);
  ...
  async getByName(name) {
    const accounts = this.db.exec("SELECT * FROM accounts WHERE name = ?", [
      name,
    ]);
   ...
   */
  async getActive() {
    const accounts = this.db.exec("SELECT * FROM accounts WHERE is_active = 1");
  ...
  async search(query) {
    ...
    return Promise.all(accounts.map((acc) => this.getById(acc.id)));
  }
  ...
  async restoreAutoSave() {
    try {
      ...
      if (permission !== "granted") {
        ...
        const newPermission = await saved.requestPermission({
          mode: "readwrite",
        });
...
<div class="settings-panel">
  ...
  <div class="setting-item">
    ...
    <p class="hint">
      Database will auto-save to your chosen folder every 5 minutes
    </p>
  </div>
...
document
  .getElementById("setup-auto-save-btn")
...
function showMigrationDialog() {
  ...
  document
    .getElementById("migrate-now-btn")
    .addEventListener("click", async () => {
  ...
  document
    .getElementById("skip-migration-btn")
...
describe("Full User Flow", () => {
  it("should handle complete account lifecycle", async () => {
    ...
    const account = await accountService.create({
      ...
      email: "test@example.com",
      cookies: [{ name: "session", value: "abc123", domain: ".cursor.sh" }],
    });
