...
(function () {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    // Validate sender to ensure messages originate from this extension only
    const isTrustedSender = !!sender && sender.id === chrome.runtime.id;
    if (request.type === "evaluateExpression" && isTrustedSender) {
      try {
        ...
        // Remove unsafe eval fallback entirely
        sendResponse({ success: false, error: error.toString() });
      }
